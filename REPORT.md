...
* Being an environment provisioning bot the project not only helped us to develop our software development but also our DevOps concepts from the start. The testing of the bot helped us learn how to interact with multiple instance providers such as AWS and Digital Ocean.
...
* Deployment of bot - Initially, it was planned to deploy bot on [beepboophq.com](https://beepboophq.com), where the bot would be deployed using the code base on github repository with the help of git web hooks. Since beepboophq.com uses Docker Containers, we were successful in developing Docker images which had the capability to host the bot. But later during testing, it was found that, beepboophq.com didn't provide any persistent storage as a result after every crash our database was reinitialized. We realized this after we contacted support team from beepboophq.com. Hence, we decided to move to AWS for deploying and hosting bot.
